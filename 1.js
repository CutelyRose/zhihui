...
function getCheckChar$1(e) {
        var b = new Buffer.from(d(e, "£¬¡£fdjf,jkgfkl"));
...
function get_session(){
    var r = "Win10/Chrome122/".concat(createUuid$1());
    t="".concat(getCheckChar$1(r)).concat('w').concat(r)
    return t
...
function DES_Encrypt(a,b,c) {
    if(c=='0')
        word=a
    else
        word=d(a,c);
    var key = CryptoJS.enc.Utf8.parse(b);
...
function DES_Decrypt(ciphertext, b) {
    // b 为 8 字节密钥
    var key  = CryptoJS.enc.Utf8.parse(b);

    // 先 Base64 解码得到 WordArray
    var encryptedHexStr = CryptoJS.enc.Base64.parse(ciphertext);
